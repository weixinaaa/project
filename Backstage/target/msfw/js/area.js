...
function change(v, statu) {
	var str = "0";
	for(i = 0; i < v; i++) {
		str += ("_" + (document.getElementById(s[i] + statu).selectedIndex - 1));
	};
	var ss = document.getElementById(s[v] + statu);
	with(ss) {
		length = 0;
		options[0] = new Option(opt0[v], opt0[v]);
		if(v && document.getElementById(s[v - 1] + statu).selectedIndex > 0 || !v) {
			if(dsy.Exists(str)) {
				ar = dsy.Items[str];
