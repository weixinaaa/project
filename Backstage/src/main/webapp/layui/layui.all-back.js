...
! function() {
	...
	w.prototype = [], w.prototype.constructor = w, T.extend = function() {
		...
		e.remove(), a ? t.elem.append(i) : (document.body.appendChild(i), e.position()), e.checkDate().calendar(), e.changeEvent(), D.thisElem = e.elemID, "function" == typeof t.ready && t.ready(t.dateTime)
		...
		return l = l || r.value, "string" == typeof l && (l = l.replace(/\s+/g, " ").replace(/^\s|\s$/g, "")), "string" == typeof l && l ? i.EXP_IF.test(l) ? r.range ? (l = l.split(" " + r.range + " "), i.startDate = i.startDate || i.systemDate(), i.endDate = i.endDate || i.systemDate(), r.dateTime = T.extend({}, i.startDate), T.each([i.startDate, i.endDate], function(e, t) {
			m(t, l[e], e)
		...
		var t, a, i, r = this,
			...
			y = e ? 1 : 0,
			h = T(r.table[y]).find("td"),
			f = T(r.elemHeader[y][2]).find("span");
		if(l.year < d[0] && (l.year = d[0], r.hint("最低只能支持到公元" + d[0] + "年")), l.year > d[1] && (l.year = d[1], r.hint("最高只能支持到公元" + d[1] + "年")), r.firstDate || (r.firstDate = T.extend({}, l)), c.setFullYear(l.year, l.month, 1), t = c.getDay(), a = n.getEndDate(l.month, l.year), i = n.getEndDate(l.month + 1, l.year), T.each(h, function(e, n) {
		...
		if(h && y.removeChild(h), y.appendChild(d), "year" === e || "month" === e) T(n.elemMain[t]).addClass("laydate-ym-show"), T(d).find("li").on("click", function() {
			...
			if(!T(this).hasClass(s)) {
				...
				"year" === a.type || "month" === a.type ? (T(d).find("." + o).removeClass(o), T(this).addClass(o), "month" === a.type && "year" === e && (n.listYM[t][0] = r, l && (n[["startDate", "endDate"][t]].year = r), n.list("month", t))) : (n.calendar(), n.closeList()), n.setBtnStatus(), a.range || n.done(null, "change"), T(n.footer).find(v).removeClass(s)
			}
		});
		else {
			var S = T.elem("span", {
				...
				k = T(m[2]).find("." + g);
			H(), S.innerHTML = a.range ? [r.startTime, r.endTime][t] : r.timeTips, T(n.elemMain[t]).addClass("laydate-time-show"), k[0] && k.remove(), m[2].appendChild(S), T(d).find("ol").each(function(e) {
				var t = this;
				T(t).find("li").on("click", function() {
					var r = 0 | this.innerHTML;
					T(this).hasClass(s) || (a.range ? n[x][C[e]] = r : i[C[e]] = r, T(t).find("." + o).removeClass(o), T(this).addClass(o), n.setBtnStatus(null, T.extend({}, n.systemDate(), n.startTime), T.extend({}, n.systemDate(), n.endTime)), E(), H(), (n.endDate || "time" === a.type) && n.done(null, "change"))
				})
			})
		}
		return n
		...
		if(a.range && !n.endDate && T(n.footer).find(p).addClass(s), n.endDate) return e = n.newDate({
			...
			T(i).removeClass(m + " " + o), s !== e && s !== t || T(i).addClass(T(i).hasClass(y) || T(i).hasClass(h) ? m : o), s > e && s < t && T(i).addClass(m)
		})
		...
		if(r = {
			...
			} else "datatime" !== n.type && "static" !== n.position ? (l(!0), t.setValue(t.parse()).remove().done()) : (l(!0), t.calendar().done().done(null, "change"))
		...
		return {
			prevYear: function() {
				s("sub") || (a.year--, t.calendar(), n.range || t.done(null, "change"))
			},
			prevMonth: function() {
				...
				T.extend(a, {
					year: e[0],
					month: e[1]
				}), t.calendar(), n.range || t.done(null, "change")
			},
			nextMonth: function() {
			...
			nextYear: function() {
				s() || (a.year++, t.calendar(), n.range || t.done(null, "change"))
			}
		}
...
layui.define("layer", function(e) {
	...
	f.prototype.config = {
		...
		var o, a = this,
			...
			d = function(e, i) {
				a.elemFile.next("." + s).remove(), r.value = "";
				try {
					if(typeof(i)=="string")
					i = JSON.parse(i)
				} catch(t) {
...
layui.define(["laytpl", "laypage", "layer", "form"], function(e) {
	...
	var t = layui.$,
		...
		T = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">', "{{# var fixed, right; }}", '<div class="layui-table-header">', z(), "</div>", '<div class="layui-table-body layui-table-main">', A, "</div>", '{{# if(fixed && fixed !== "right"){ }}', '<div class="layui-table-fixed layui-table-fixed-l">', '<div class="layui-table-header">', z({
			fixed: !0
		}), "</div>", '<div class="layui-table-body">', A, "</div>", "</div>", "{{# }; }}", "{{# if(right){ }}", '<div class="layui-table-fixed layui-table-fixed-r">', '<div class="layui-table-header">', z({
			fixed: "right"
		}), '<div class="layui-table-mend"></div>', "</div>", '<div class="layui-table-body">', A, "</div>", "</div>", "{{# }; }}", "{{# if(d.data.page){ }}", '<div class="layui-table-tool">', '<div class="layui-inline layui-table-page" id="layui-table-page{{d.index}}"></div>', "</div>", "{{# } }}", "<style>", "{{# layui.each(d.data.cols, function(i1, item1){", "layui.each(item1, function(i2, item2){ }}", ".laytable-cell-{{d.index}}-{{item2.field||i2}}{ width:{{item2.width||50}}px }", "{{# });", "}); }}", "</style>", "</div>"].join(""),
		D = t(window),
		F = t(document),
		j = function(e) {
			var i = this;
	...
	j.prototype.config = {
		...
		c.sortKey = {
			field: n,
			sort: i
		}, "asc" === i ? s = layui.sort(s, n) : "desc" === i ? s = layui.sort(s, n, !0) : (s = c.cacheData, delete c.sortKey), c.renderData({
			data: s
