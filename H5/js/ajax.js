...
function ziliao() {
    ...
    $.ajax({
        ...
        success: res => {
            ...
            let html = '';
            html += '<img src="' + res.data.img + '" style="width: 30%;border-radius: 100%">' +
                '<div class="list">' +
                '<span>姓名</span>' +
                '<input type="text" name="" class="input" placeholder="' + res.data.name + '" id="name">' +
                '</div>' +
                '<div class="list">' +
                '<span>联系电话</span>' +
                '<input type="text" name=""  class="input" placeholder="' + res.data.tel + '" id="tel">' +
                '</div>' +
                '<div class="list">' +
                '<span>性别</span>' +
                '<div class="sex">' +
                ...
                '<div class="list">' +
                '<span>邮箱</span>' +
                '<input type="text" name="" placeholder="' + res.data.mailbox + '" class="input" id="email">' +
                '</div>' +
                '<div class="list">' +
                '<span>我的邀请码</span>' +
                '<input type="number" name="" value="' + res.data.myInvitationCode + '" class="input" readonly>' +
                '</div>' +
                '<div class="list">' +
                '<span>邀请人邀请码</span>' +
                '<input type="number" maxlength="6" min="000001" pattern="[0-9]" name="" value="' + res.data.invitationcode + '" class="input" id="yaoqing">' +
                '</div>'
        ...
        success: res => {
            ...
            for (i = 0; i < res.data.length; i++) {
                ...
                html += '<div class="list-left">\n' +
                    ...
                    '\t\t</div>\n' +
                    '\t\t<p class="money">' + res.data[i].amountOfMoney + '</p>\n' +
                    '\t\t<div class="right">\n' +
                    '\t\t\t<img src="' + imgurl + '" >\n' +
                    '\t\t\t<p >' + sh + '</p>\n' +
                    '\t\t</div>\n' +
...
function list_guanggao() {
    ...
    $.ajax({
        ...
        success: res => {
            ...
            //  }
            for (i = 0; i < gg.length; i++) {
                html += '<img src="./image/icon/look.png" alt="" style="width: 100%;background: white;padding: 10px 0;">'
            }
...
function guanggao() {
    ...
    $.ajax({
        ...
        success: res => {
            ...
            let html = '';
            html += '<p class="title">hhhh</p>\n' +
                '\t<div class="top">\n' +
                '\t\t<span class="time">' + time + '</span>\n' +
                '\t\t<b class="number">点击量：' + res.data.clicks + '</b>\n' +
                '\t</div>' +
        ...
        success: res => {
            ...
            for (i = 0; i < res.data.length; i++) {
                ...
                html += '<div class="list">\n' +
                    ...
                    '\t\t\t<p>' + time + '</p>\n' +
                    '\t\t</div>\n' +
                    '\t\t<span class="money">+' + res.data[i].profit + '元</span>\n' +
                    '\t</div>'
            }
...
function qian() {
    $.ajax({
        ...
        success: res => {
            console.log(res)
            let html = '';
            html += '<p class="money">￥' + res.data + '.00</p>'
            document.getElementById('qian').innerHTML = html
    ...
    $.ajax({
        ...
        success: res => {
            ...
            html1 += '<img src="' + res.data[0].img + '">\n' +
                '\t\t<div class="right">\n' +
                '\t\t\t<div>\n' +
                '\t\t\t\t<p>' + res.data[0].charterPrice + '</p>\n' +
                '\t\t\t\t<p>费用(元)/天</p>\n' +
                '\t\t\t</div>\n' +
                '\t\t</div>'
            html2 += '<img src="' + res.data[1].img + '">\n' +
                '\t\t<div class="right">\n' +
                '\t\t\t<div>\n' +
                '\t\t\t\t<p>' + res.data[1].charterPrice + '</p>\n' +
                '\t\t\t\t<p>费用(元)/天</p>\n' +
                '\t\t\t</div>\n' +
                '\t\t</div>'
            html3 += '<img src="' + res.data[2].img + '">\n' +
                '\t\t<div class="right">\n' +
                '\t\t\t<div>\n' +
                '\t\t\t\t<p>' + res.data[2].charterPrice + '</p>\n' +
                '\t\t\t\t<p>费用(元)/天</p>\n' +
                '\t\t\t</div>\n' +
                '\t\t</div>'
